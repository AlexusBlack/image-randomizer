...
class Translation
{
	...
	private function detectLang($default_lang) {
		...
		if(!isset($_COOKIE['translation'])) {
			$accept_language=isset($_SERVER['HTTP_ACCEPT_LANGUAGE'])?strtolower($_SERVER['HTTP_ACCEPT_LANGUAGE']):$default_lang;
	...
	public function add($name, $langArray) {
		...
		//Добавляем универсальные значения
		$this->translations[$name]['PHP_SELF']=$_SERVER['PHP_SELF'];
		$this->translations[$name]['CURRENT_YEAR']=date("Y");	
	}
